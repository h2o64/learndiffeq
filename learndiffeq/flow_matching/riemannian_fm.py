# Implementation of Riemannian Flow Matching
...
class RiemannianFlowMatching(LearnODE):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if per_species_ot:
            ...
            if self.hparams.use_linear_assignment_particles:
                x0, _ = linear_assignment_permutation(x0, x1, L=self.hparams.L)
            elif self.hparams.ot_particles:
                x0, _ = ot_permutation(x0, x1, L=self.hparams.get('L', None))
        # Sample t uniformly
        t = torch.rand((batch_size, *self.data_shape_ones), device=self.device)
        # Compute x_t
        x_t = exp_map(x1, (1. - t) * log_map(x1, x0, self.hparams.L), self.hparams.L)
        # Compute the loss
        loss = torch.sum(torch.square(self.b(t, x_t, a=a1) - (log_map(x_t, x1, self.hparams.L) / (1. - t))),
    ...
    def sample(self, a, values, **kwargs):
        ...
        """
        ret = super().sample(
            torch.remainder(values, self.hparams.L),
            b_kwargs={'a': a},
            **kwargs
        )
...
class RiemannianFlowMatchingWithTwoDatasets(RiemannianFlowMatching):
